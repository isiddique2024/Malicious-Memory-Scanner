namespace verification {
    ...
    {
        imp<BOOL>("CryptCATAdminCalcHashFromFileHandle", file_handle, hash_size, nullptr, 0);
        *hash = new unsigned char[*hash_size];

        if (!(imp<BOOL>("CryptCATAdminCalcHashFromFileHandle", file_handle, hash_size, *hash, 0))) {
            delete[] * hash;
            return false;
        }
    ...
    {
        HCATADMIN cat_admin{};
        if (!(imp<BOOL>("CryptCATAdminAcquireContext", &cat_admin, 0, 0))) {
            return false;
        }

        CATALOG_INFO catalog_info_{};
        const auto catalog_info = imp<HCATINFO>("CryptCATAdminEnumCatalogFromHash", cat_admin, hash, hash_size, 0, nullptr);

        bool has_catalog_signature = catalog_info && imp<BOOL>("CryptCATCatalogInfoFromContext", catalog_info, &catalog_info_, 0);

        if (catalog_info) {
            imp<BOOL>("CryptCATAdminReleaseCatalogContext", cat_admin, catalog_info, 0);
        }
    ...
    auto verify_dll(const std::wstring& file_name) -> bool
    {
        ...
        const auto file_handle = imp<void*>(
            "CreateFileW",
            file_name.c_str(),
            GENERIC_READ,
            FILE_SHARE_READ,
